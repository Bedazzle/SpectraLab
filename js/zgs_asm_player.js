...
;   DEFINE ZGS_USE_FLOOD        ; ~2166 bytes — flood fill (0x19, 0x77) — includes 1280 bytes of buffers
...
; ==========================================================================
; flood_fill — two-phase scanline flood fill
; Phase 1: solid fill (0xFF) using screen as visited bitmap
; Phase 2: apply pattern mask to filled area
; ==========================================================================
flood_fill:
...
; ---- Phase 1: Solid fill using screen-as-visited ----
.main_loop:
    ; Check if stack empty
    ld hl, (ff_sp)
    ld a, h
    or l
    jp z, .phase2               ; stack empty → apply pattern

    ; Pop (x, y)
    call ff_pop

    ; Check bounds and pixel
    ld a, (ff_pop_y)
    cp 192
    jp nc, .main_loop
    ld a, (ff_pop_x)
    ...
    call check_pixel
    or a
    jp nz, .main_loop           ; already set = boundary or visited
...
.right_done:
    ...
    ; Fill span with solid 0xFF (this also marks as "visited")
    call fill_span_solid
...
; ---- Phase 2: Apply pattern mask to filled pixels ----
; For each screen row 0..191, AND each byte with the pattern row byte.
; Only the filled area (0xFF bytes from Phase 1) will be affected.
; Pre-existing pixels (from before the fill) are also preserved correctly
; since they were treated as boundaries and not overwritten in Phase 1.
.phase2:
    xor a
    ld (ff_pop_y), a            ; reuse as row counter

.p2_row_loop:
    ; Compute pattern byte for this row
    ld a, (cur_pat)
    rlca
    rlca
    rlca                            ; A = cur_pat * 8
    ...
    add hl, bc
    ld a, (hl)
    ld (ffs_pat_row), a             ; pattern byte for this row
    ...
    rlca
    rlca
    ld l, a                         ; HL = screen address of column 0

    ; AND each of 32 bytes with pattern
    ld b, 32
    ld a, (ffs_pat_row)
    ld c, a                         ; C = pattern byte
.p2_col_loop:
    ld a, (hl)
    and c                           ; apply pattern mask
    ld (hl), a
    inc l                           ; next column (same row)
    djnz .p2_col_loop
...
; ==========================================================================
; fill_span_solid — fill span ff_lx..ff_rx at ff_pop_y with solid 0xFF
...
fill_span_solid:
    ...
    ; Single column: combine left and right masks
    ld a, (ff_lx)
    and 0x07
    ld c, a
    ld b, 0
...
.sol_middle:
    ; Fill middle full bytes with 0xFF
    ld a, (ffs_cur_col)
    ld b, a
    ld a, (ffs_right_col)
    cp b
...
.skip_y:
    ...
    IFDEF ZGS_USE_FLOOD
; fill_span_solid temporaries (used by flood_fill routines above)
ffs_h:          db 0
ffs_l_base:     db 0
ffs_left_col:   db 0
ffs_right_col:  db 0

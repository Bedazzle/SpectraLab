...
/**
 * Syncs a Picture back into screenData (format-specific interleaved layout).
 * Dispatches based on picture.sourceFormat.
 * @param {Picture} picture - Source picture
 * @param {Uint8Array} scrData - Target screenData buffer
 */
function syncScreenDataFromPicture(picture, scrData) {
  const fmt = picture.sourceFormat;
  const cols = picture.cols;
  const height = picture.height;

  if (fmt === '53c') {
    // Pattern mode: write 768 attrs at offset 0
    const attrs = picture.planes[0].attrs;
    for (let i = 0; i < 768; i++) {
      scrData[i] = attrs[i];
    }
  ...
  if (fmt === 'specscii') {
    // Text mode: copy grids to global specscii vars
    if (picture.chars && typeof specsciiCharGrid !== 'undefined') {
      if (specsciiCharGrid) specsciiCharGrid.set(picture.chars);
    }
    if (picture.cellColors && typeof specsciiAttrGrid !== 'undefined') {
      if (specsciiAttrGrid) specsciiAttrGrid.set(picture.cellColors);
    }
    if (picture.cellMask && typeof specsciiMask !== 'undefined') {
      if (specsciiMask) specsciiMask.set(picture.cellMask);
    }
    return;
  }
  ...
  if (fmt === 'img' || fmt === 'mgh' || fmt === 'hlr') {
    ...
    scrData.set(bm1, 0);
    for (let i = 0; i < attrSize; i++) {
      scrData[6144 + i] = picture.planes[0].attrs[i];
    }
    const bm2 = interleaveBitmap(picture.planes[1].bitmap, 256, 192);
    scrData.set(bm2, frameSize);
    for (let i = 0; i < attrSize; i++) {
  ...
  if (fmt === 'rgb3') {
    // RGB3: three bitmaps
    const bmR = interleaveBitmap(picture.planes[0].bitmap, 256, 192);
    scrData.set(bmR, 0);
    const bmG = interleaveBitmap(picture.planes[1].bitmap, 256, 192);
    scrData.set(bmG, 6144);
    const bmB = interleaveBitmap(picture.planes[2].bitmap, 256, 192);
    scrData.set(bmB, 12288);
    return;
  }
  ...
  if (fmt === 'zxp' || fmt === 'ch$' || fmt === 'mgh') {
    ...
    for (let f = 0; f < frameCount; f++) {
      ...
      const bitmap = picture.planes[f].bitmap;
      for (let i = 0; i < bitmapSize; i++) {
        scrData[off + i] = bitmap[i];
      }
      const attrs = picture.planes[f].attrs;
      for (let i = 0; i < attrSize; i++) {
        scrData[off + bitmapSize + i] = attrs[i];
      }
    }
    return;
  }
  ...
  if (fmt === 'bmc4') {
    // BMC4: bitmap + two attr banks + border
    const scrBitmap = interleaveBitmap(picture.planes[0].bitmap, 256, 192);
    scrData.set(scrBitmap, 0);
    const attrs = picture.planes[0].attrs;
    for (let r = 0; r < 24; r++) {
      for (let c = 0; c < 32; c++) {
        scrData[6144 + r * 32 + c] = attrs[(r * 2) * 32 + c];
        scrData[6912 + r * 32 + c] = attrs[(r * 2 + 1) * 32 + c];
      }
    }
    if (picture.border) {
      writeBorder(picture.border, scrData, 7680);
    }
    return;
  }
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let col = 0; col < cols; col++) {
      const scrOffset = thirdBase + col + charRow * 32 + pixelLine * 256;
      scrData[scrOffset] = bitmap[y * cols + col];
    }
  }

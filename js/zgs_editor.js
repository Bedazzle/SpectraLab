...
class ZgsDisassembler {
  ...
  disassemble() {
    ...
    while (this.pc < this.data.length) this._disasmOpcode();
...
class ZgsAssembler {
  ...
  assemble() {
    ...
    // Always append END opcode (0x18) to guarantee the scene terminates.
    ...
    // A redundant END after an explicit one is harmless (1 extra byte).
...
function zgsOpenFile(file) {
  ...
  reader.onload = function () {
    ...
    const curText = cur.sourceText.trim();
    const isDefault = curText === '' ||
      /^;\s*(New ZGS scene|Scene\s+\d+)\s*\n/.test(cur.sourceText) &&
      cur.undoStack.length === 0;
...
function handleZgsExportAsm() {
  ...
  for (let i = 0; i < zgsScenes.length; i++) {
    ...
    compiled.push({ name: scene.name, safeName: zgsSafeFilename(scene.name), data, usesPacked, usesText, usesText42, usesText64 });
  }
  ...
  let asmText = '; ZGS Player \u2014 generated by SpectraLab\n'
    ...
    + '    DEFINE ZGS_USE_FLOOD        ; ~1500 bytes \u2014 flood fill (0x19, 0x77) \u2014 includes 512 bytes of stack buffer\n'
...
function zgsLoadProject(jsonText) {
  ...
  zgsScenes = project.scenes.map((/** @type {{name: string, sourceText: string}} */ s) => {
...
function zgsDrawOverlay() {
  ...
  if (zgsDrawing && zgsDrawStart && zgsDrawEnd) {
    ...
    const sx = zgsDrawStart.lx * 2, sy = zgsDrawStart.ly * 2;
    const ex = zgsDrawEnd.lx * 2, ey = zgsDrawEnd.ly * 2;
...
function zgsDocMouseMoveDuringDrag(e) {
  const coords = zgsCanvasToLogical(e);
  if (!coords) return;
  // Active drag: update rubber band end
  if (zgsDrawing && zgsDrawStart) {
    zgsDrawEnd = { lx: coords.lx, ly: coords.ly };
    zgsDrawOverlay();
    return;
...
function zgsCanvasMouseUp(e) {
  ...
  if (zgsDrawEnd) {
    ...
    if (zgsTool === 'line' && zgsDrawButton === 2) {
      ...
      const instr = zgsGenerateInstruction(zgsTool, zgsDrawStart, zgsDrawEnd);
